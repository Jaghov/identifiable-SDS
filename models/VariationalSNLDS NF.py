...
class FlowSNLDS(nn.Module):
    ...
    def __init__(self, obs_dim, latent_dim, hidden_dim, num_states, beta=1, encoder_type='recurent', device='cpu', annealing=False, inference='alpha'):
        ...
        if self.encoder_type=='factored':
            ...
            self.encoder = MLP(obs_dim, 2*latent_dim, hidden_dim, 'leakyrelu').to(device).float() # Replace with Glow
            ...
            self.encoder_mean_var = nn.Linear(hidden_dim, 2*latent_dim).to(device).float() # replace this with Glow
        ...
        elif encoder_type == 'flow':
            self.encoder = Glow(n_channels=2, n_steps=16, n_flow_blocks=1, dequantize=False, input_type='factored').to(device)
            self.encoder_causal = nn.LSTM(obs_dim, obs_dim, num_layers=2, batch_first=True, bidirectional=False).to(device).float() # ASK Why causal after bidirectional?
        # Decoder p(x|z) # IMPL NO LONGER NECESSARY
        if self.encoder_type=='video': 
            self.decoder = CNNFastDecoder(latent_dim, 3, n_feat, n_layers=n_layers).to(device).float()
        elif self.encoder_type == 'recurrent' or self.encoder_type== 'factored':
            self.decoder = MLP(latent_dim, obs_dim, hidden_dim, 'leakyrelu').to(device).float()
        elif self.encoder_type == 'flow':
    ...
    def _encode_obs(self, x):
        """VRNN: Encodes a sequence of observations as a pair of latent
        ...
        if self.encoder_type=='factored':
            ...
            (B, T, D) = x.shape
            (z_list, log_det_jacobian) = self.encoder(x.reshape(B*T,-1)).reshape(B,T,-1)
            z = self.encoder_causal(self.encoder.list_to_z(z_list))
    ...
    def _compute_local_evidence(self, z):
        """Computes the p(z_{1:T} | s_{1:T} ) for all states
        """
        T = z.size(1)
    ...
    def _alpha(self, local_evidence):
        """ p(z_1, ... z_n | s_n) using dynamic programming
        """
    ...
    def _beta(self, local_evidence, log_Z):
        """ p(z_n+1, ... z_N | s_n) using dynamic programming
        """
        N, T, _ = local_evidence.shape
    ...
    def _compute_posteriors(self, log_evidence):
        """Computes the p(s_{1:T} | z_{1:T} ) for all states
        """
