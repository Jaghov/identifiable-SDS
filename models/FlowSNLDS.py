...
class FlowSNLDS(NeuralSNLDS):
    ...
    def __init__(self, obs_dim, latent_dim, hidden_dim, num_states, steps=16, blocks=1, encoder_type='factored', device='cpu', annealing=False, inference='alpha', n_bits=5):
        ...
        if encoder_type == 'factored':
            ...
            self.encoder = Glow(n_channels=3, n_steps=steps, n_flow_blocks=blocks, dequantize=True, input_type='image', n_bits=self.n_bits).to(device).float()
    ...
    def _encode_obs(self, x, train=True):
        ...
        if self.encoder_type == "factored":
            ...
            (B, T, C, H, W)  = x.size()
            (z_list, log_det_jacobian) = self.encoder(x.reshape(B*T, C, H, W))
    ...
    def forward(self, x, train=True):
        ...
        if self.encoder_type == 'video':
            x_hat = self._decode(z_sample.reshape(B*T, C, H, W )).reshape(B, T, C, H, W)
        else:
